...
def main(cutoff_score, validate, downsample, tsne):
    ...
    if validate:
        ...
        print(np.sum(true_doublet_labels[np.where(true_scores >= cutoff_score)[0]]) / (
            float(len(true_scores[np.where(true_scores >= cutoff_score)[0]]))))
        # return
    else:
        ...
        # Bar chart of scores per community
        ax2 = plt.subplot(122)
        labels, values = zip(*raw_com_count.items())
        indexes = np.arange(len(labels))
        width = 0.75
...
if __name__ == '__main__':
    ...
    main(options.cutoff_score, options.validate, options.downsample, options.tsne)
